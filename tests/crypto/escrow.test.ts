...
describe("escrow（二重ラップとリカバリ）", () => {
  it("運用パスフレーズとリカバリコードの両方から同一 DEK を復元できる", () => {
    const dek = randomDek()
    const pass = "op-pass-12345"
    const code = generateRecoveryCode()
    const file = buildKeyringFile(dek, pass, code)

    const viaOp = unwrapDek(file.op.wrappedDek, pass, Buffer.from(file.op.salt, "base64url"))
  ...
  it("リカバリで新運用PFに再ラップ後、旧PF失敗・新PF成功・DEK不変", () => {
    const dek = randomDek()
    const code = generateRecoveryCode()
    const file = buildKeyringFile(dek, "old-pass-12345", code)
    ...
    file.op = {
      ...
      wrappedDek: wrapDek(recovered, "new-pass-67890", newSalt),
    }

    // 旧PFは失敗
    expect(() => unwrapDek(file.op.wrappedDek, "old-pass-12345", newSalt)).toThrow()
    // 新PFは成功し、同一 DEK
    const viaNew = unwrapDek(file.op.wrappedDek, "new-pass-67890", newSalt)
