...
### シークレットの受け渡し（実装者向け・重要）

**要点: DEK とリカバリコードはコードが乱数生成するもの。人が用意・入力するのは運用パスフレーズだけ。** DEK は「渡す」ものではなく、パスフレーズ（またはリカバリコード）からラップを解いてメモリ上に毎回「導出」する。

| シークレット | 誰が作る | 人が入力するか | 保存形態 |
|---|---|---|---|
| **DEK** | コードが乱数生成（`randomBytes(32)`、init で1回） | ❌ 人は触らない | ディスクには**ラップ済み**のみ。実体はメモリだけ |
| **運用パスフレーズ** | 人が決める | ✅ init で設定、起動毎に入力 | 保存しない（scrypt由来のKEKが一時的にメモリに乗るだけ） |
| **リカバリコード** | コードが乱数生成（init で1回表示） | △ リカバリ時のみ入力 | サーバに保存しない。人がオフライン保管 |

**局面別**:
- **開発・P0テスト**: テスト内で `randomBytes` により使い捨ての DEK / パスフレーズ / keyring.json（一時パス）を生成。**本番シークレットをコードや `.env` に書かない**。
- **初回セットアップ（本番サーバで1回）**: `npm run keyring:init` を対話実行。人が運用パスフレーズを入力 → DEK は内部で生成・即ラップ・破棄（画面に出さない）→ リカバリコードを**1回だけ**表示し保管を促す。
- **毎回の起動時**: `npm run keyring:unlock` で運用パスフレーズのみ入力。

**渡し方のルール（P0で必ず守る）**:
1. パスフレーズ/リカバリコードは **no-echo の stdin プロンプト**で受ける（`readline` でマスク入力）。
2. **コマンドライン引数・環境変数で渡さない**（プロセス一覧・シェル履歴・`.env` に残るため）。
3. **scrypt + ラップ解除はサーバ側で実行**。CLI はパスフレーズを `http://127.0.0.1` の unlock エンドポイントへ**転送するだけ**（loopback なので平文転送可。生の DEK は配線に乗せない）。
4. `keyring.json` に**平文 DEK が絶対に入らない**ことを受け入れ基準1でテスト。
